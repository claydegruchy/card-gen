...
Squib::Deck.new cards: data['Title'].size, layout: %w(economy.yml) do
  ...
  text str: data['Type'].map { |x| x.downcase.titlecase }, layout: :middle_left

  text str: data['Price'], layout: :upper_right

  text str: data['Enc'], layout: :upper_left

  text str: data['Availability'], layout: :middle_right

  text str: data['Reach'], layout: :lower_left

  text str: data['Damage'].map { |e| e.to_s.gsub(/[^0-9a-z+ ]/i, '') }, layout: :lower_right

  # text str: data['Qualities and Flaws'].map { |x| x.split('|').join("\n") }, layout: :desc
  # text str: data['Qualities and Flaws'].map { |x| x.split('|').map { |e| weaponqualities[e]  }.join("\n") }, layout: :desc
  text str: data['Qualities and Flaws'].map { |x| x.split('|').map { |e| " <span font_size='x-large'>#{e}</span> (#{weaponqualities[e.titlecase].to_s})" }.join("\n") }, layout: :desc

  text str: names, layout: :copyright
